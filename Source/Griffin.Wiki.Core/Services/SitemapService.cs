...
namespace Griffin.Wiki.Core.Services
{
    ...
    public class SiteMapService
    {
        ...
        public SiteMapService(PageTreeRepository pageTreeRepository)
        {
        ...
        public IEnumerable<SiteMapNode> Get(string pageUri)
        {
            ...
            {
                var node = new SiteMapNode(pageTreeNode.Page.Title, pageTreeNode.CreateLink(pageUri), pageTreeNode.CreateLinkPath(pageUri));
                nodes.Add(node);
        ...
        private void AddChildren(string pageUri, SiteMapNode currentMapNode, WikiPageTreeNode currentTreeNode, IEnumerable<WikiPageTreeNode> completeTree)
        {
            foreach (var childTreeNode in completeTree.Where(x=>x.ParentLinage==currentTreeNode.Lineage))
            {
                var child = new SiteMapNode(childTreeNode.Page.Title, childTreeNode.CreateLink(pageUri), childTreeNode.CreateLinkPath(pageUri));
